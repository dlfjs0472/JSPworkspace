...
//서비스 인터페이스를 구현한 구현클래스
//왜 서비스에 인터페이스까지 둬야하는지 의문이 생기겠지만, 다음주까지 참자..
//이유: 어플리 케이션의 의존성을 약화시키기위함..(DI를 배워야 이해가능함..)
public class MemberServiceImpl implements MemberService{
	private PoolManager pool=PoolManager.getInstance(); //DAO들 사이의 Connection을 공유하기위해..
	//부장님이 일시킬 모델 객체들...
	...
	public void regist(Member member, HttpServletRequest request) throws LicenseRegistException{
		Connection con = pool.getConnection(); //DAO들에게 나눠줄 커넥션 얻기!
		((JdbcMemberDAO)memberDAO).setCon(con); //Connection 객체 주입
		((JdbcLicenseDAO)licenseDAO).setCon(con);// Connection 객체 주입
		//jdbc에서의 Connection은 autoCommit이 true로 설정되어있다.. 오라클과 틀림
