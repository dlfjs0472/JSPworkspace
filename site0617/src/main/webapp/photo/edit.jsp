...
<%
	//jsp에서는 서블릿에서 사용되는 여러객체들을 내장객체 형태로 즉 이름을 이미 부여해 놓은 형태로 지원한다!!
	...
	if(myfile!=null){
		...
		//기존 파일 삭제 + 새로운 업로드된 파일의 파일명을 현재 날짜조합을 이용하여 rename
		File oldFile=new File(path+"/"+filename);
		if(oldFile.delete()){ //기존파일이 지워졌다면?
			File newFile = multi.getFile("myfile"); //이미 data 디렉토리에 업로드된 파일(아직 현재날짜 조합으로 처리 안되어있음)
