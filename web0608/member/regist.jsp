...
<%
	//[3]스트립틀릿 영역
	//여기에 작성한 코드는 메서드에 작성한 코드로 간주되므로, jsp에서 사용할 로직 등을 
	//여기서 작성한다!!	
	//out.print("hello");

	//클라이언트가 전송한 파라미터들을 받아서 변수에 보관해보자!!
	//클라이언트가 서버에 현재 regist.jsp를 요청할때 전송한 파라미터를 받아보자
	//jsp는 기본적으로, 개발자가 생성하지 않아도 이미 시스템에 의해 생성된 내장 객체를 지원
	//하는데, 특히 클라이언트가 전송한 요청정보와 관련된 작업을 위해서는 request 내장객체를
	//이용하면 된다 request에는 클라이언트의 모든 요청과 관련된 정보가 들어있다
	//request.getParameter("html파라미터명" =name);
	...
	*/
	...
	//jsp의 스크립틀릿 영역은, 이미 내부적으로 예외처리가 되어 있기 때문에
